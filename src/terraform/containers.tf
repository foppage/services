...
resource "docker_container" "runner" {
  ...
  restart = "unless-stopped"
  command = ["sh", "-c", "forgejo-runner daemon --url https://git.june.pet/ --uuid ${var.forgejo_runner_uuid} --token-url file://<(echo -n '${var.forgejo_runner_token}') --label docker:docker://node:lts --label ubuntu-latest:docker://node:lts"]
