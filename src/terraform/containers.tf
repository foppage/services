...
resource "docker_container" "runner" {
  ...
  restart = "unless-stopped"
  command = ["sh", "-c", "forgejo-runner daemon --url http://forgejo:3000/ --uuid ${var.forgejo_runner_uuid} --token-url file://<(echo -n '${var.forgejo_runner_token}') --label docker:docker://node:lts --label ubuntu-latest:docker://node:lts"]
